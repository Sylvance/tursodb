...
RSpec.describe Tursodb::AuthenticationApi::RevokePlatformApiTokenForUser do
  ...
  describe "#call" do
    context "when the API call is successful" do
      ...
      before do
        stub_request(:delete, "#{config.base_url}/v1/auth/api-tokens/#{body[:name]}")
      ...
      it "returns a Result object with tokens" do
        ...
        expect(results).to be_a(Tursodb::AuthenticationApi::RevokePlatformApiTokenForUser::Result)
        expect(results.token).to be_an(Tursodb::AuthenticationApi::Resources::PlatformApiToken)
        expect(results.token.token).to eq("token-value")
      end
    end
    ...
    context "when the API call fails" do
      ...
      context "when status is 401" do
        ...
        it "returns an ErrorResponse object with an error message" do
          ...
          expect(error_response).to be_a(Tursodb::AuthenticationApi::RevokePlatformApiTokenForUser::ErrorResponse)
    ...
    context "when the API call block raises an error" do
      ...
      it "returns an ErrorResponse object with an error message" do
        results = api_caller.call(body[:name])
        expect(results).to be_a(Tursodb::AuthenticationApi::RevokePlatformApiTokenForUser::ErrorResponse)
