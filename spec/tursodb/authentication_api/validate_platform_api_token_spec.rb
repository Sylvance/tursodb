...
RSpec.describe Tursodb::AuthenticationApi::ValidatePlatformApiToken do
  ...
  describe "#call" do
    context "when the API call is successful" do
      ...
      before do
        stub_request(:get, "#{config.base_url}/v1/auth/validate")
      ...
      it "returns a Result object with tokens" do
        ...
        expect(results).to be_a(Tursodb::AuthenticationApi::ValidatePlatformApiToken::Result)
        expect(results.token).to be_a(Tursodb::AuthenticationApi::Resources::ValidateTokenExpiry)
        expect(results.token.expiry).to eq(999)
        expect(results.token.token).to eq("sample-token")
      end
    end
    ...
    context "when the API call fails" do
      ...
      context "when status is 401" do
        ...
        it "returns an ErrorResponse object with an error message" do
          ...
          expect(error_response).to be_a(Tursodb::AuthenticationApi::ValidatePlatformApiToken::ErrorResponse)
    ...
    context "when the API call block raises an error" do
      ...
      it "returns an ErrorResponse object with an error message" do
        results = api_caller.call
        expect(results).to be_a(Tursodb::AuthenticationApi::ValidatePlatformApiToken::ErrorResponse)
